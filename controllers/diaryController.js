...
diaryController.getDiaryPosts = async (req, res, next) => {
  try {
    const user = req.user
...
diaryController.getDiaryPost = async (req, res, next) => {
  try {
...
diaryController.saveDiaryPost = async (req, res, next) => {
  try {
    ...
    const post = { title, content, events, imageIds, date, createdBy: user._id }
...
diaryController.deleteDiaryPost = async (req, res, next) => {
  try {
    const { id } = req.params
...
diaryController.getImage = async (req, res, next) => {
...
diaryController.saveImages = (req, res, next) => {
  res
    .status(200)
